...
iterations_per_network = np.zeros(len(network_names), dtype='object, float')
slack_value_per_network = np.zeros(len(network_names), dtype='object, float, float')
...
for i, network_name in enumerate(network_names):
    ...
    np.savetxt(os.path.join(os.path.abspath('.'), "code", "pandapower", "results", "residuals", "{}.txt".format(network_name)), errors)
    np.savetxt(os.path.join(os.path.abspath('.'), "code", "pandapower", "results", "solution", "{}.txt".format(network_name)), x_sol)
    iterations_per_network[i][0] = network_name
    slack_value_per_network[i][0] = network_name
    iterations_per_network[i][1] = iterations
    slack_value_per_network[i][1] = S_inj[0].real
    slack_value_per_network[i][2] = S_inj[0].imag
    ...
    print("Injected active power = {:.18e}".format(S_inj[0].real))
    print("Injected reactive power = {:.18e}".format(S_inj[0].imag))
    
np.savetxt(os.path.join(os.path.abspath('.'), "code", "pandapower", "results",  "iterations.txt"), iterations_per_network, fmt=['%s', '%d'])
np.savetxt(os.path.join(os.path.abspath('.'), "code", "pandapower", "results", "slack_value.txt"), slack_value_per_network, fmt=['%s', '%.18e', '%.18e'])