...
def create_x_init(network_e=None, network_g=None,
    ...
    if number_of_clones_g == 1:
        p_init = np.linspace(0.95, 0.9, len(network_g.nodes)-1)[~np.in1d(range(len(network_g.nodes)-1), gfg_units_index)]
    else:
        ...
        p_init = p_init[~np.in1d(range(p_init.shape[0]), gfg_units_index)]
...
network_name_e = 'case9241pegase'
network_name_g = 'GasLib-4197'

slack_position_e = 0 # currently only used for naming folders, has no influence on other functions
slack_position_g = 11 # a sink node

number_of_clones_e = 1
number_of_merges_e = 0 # no limit, well the actual limit is the number of available junctions

number_of_clones_g = 1
number_of_merges_g = 0 # when number_of_clones_g >= 16, max 4 
